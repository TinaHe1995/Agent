...
def test_sequential_conversations_rebind_session_id():
    """Sequential conversations from the same agent rebind session_id.

    The session_id always reflects the most recent conversation (#3443).
    The prompt_cache_key is sticky: once set by the first conversation,
    sub-agents and sequential reuse keep it for cache-shard reuse (#2904).
    """
    agent = _agent()
    conv1 = Conversation(agent=agent)
    conv2 = Conversation(agent=agent)

    # prompt_cache_key sticks to first conversation (guard)
    assert agent.llm._call_context.prompt_cache_key == str(conv1.id)
    # session_id always reflects the latest conversation
