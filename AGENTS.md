...
- For the duplicate-candidate issue workflow, keep the auto-created label color aligned with OpenHands (`f97316` orange), and prefer surfacing invalid CLI repository input through `argparse`/`parser.error(...)` rather than raising a bare `ValueError` from `parse_args()`.
