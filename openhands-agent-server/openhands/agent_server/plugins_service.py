...
Business logic for the *plugins-only* marketplace catalog, mirroring the skills
marketplace service (``skills_service.py``). It returns only true plugins from
the OpenHands extensions marketplace — entries whose ``source`` lives under
...
from openhands.sdk.plugin import list_installed_plugins
